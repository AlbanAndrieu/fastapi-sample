...
from nabla.api.runtime_diagnostics import require_loopback
...
def test_runtime_diagnostics_accept_loopback() -> None:
    require_loopback(_request_from("127.0.0.1"))
    require_loopback(_request_from("::1"))


def test_runtime_diagnostics_reject_lan_client() -> None:
    with pytest.raises(HTTPException) as exc_info:
        require_loopback(_request_from("172.17.0.57"))
