...
def test_short_password_label_is_redacted() -> None:
    assert _redact_value("Postgres pass: example-secret") == (
        f"Postgres pass: {_REDACTED}"
    )
...
def test_connection_string_credentials_are_redacted() -> None:
    ...
    assert _redact_value(connection_url) == (
        f"{scheme}://demo:{_REDACTED}@db.example.test/app"
    )
