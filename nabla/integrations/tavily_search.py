...
def web_search(
    ...
    *,
    ...
    settings = get_settings()
    resolved = settings.web_search_max_results if max_results is None else max_results
    capped = min(max(int(resolved), 1), 5)
    return client.search(query=query, search_depth=search_depth, max_results=capped)
