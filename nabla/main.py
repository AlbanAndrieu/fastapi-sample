...
from nabla.api import appwrite_route, brave_route, google_search_route, mcp_ops_route, ping, tavily_route, v1, v2
...
from nabla.config_settings import APP_NAME, APP_PREFIX_VERSION, APP_RUNTIME_VERSION, APP_VERSION, DD_TRACE_ENABLED, OTEL_SDK_DISABLED, OTLP_GRPC_ENDPOINT, client, get_settings
...
from nabla.utils.prometheus import INFLIGHT_REQUESTS, REQUESTS, REQUESTS_IN_PROGRESS, REQUESTS_PROCESSING_TIME, RESPONSES, PrometheusMiddleware, setting_otlp, update_system_metrics
from nabla.utils.sentry_config import configure_sentry

warnings.filterwarnings("ignore", category=PydanticJsonSchemaWarning, message=".*non-serializable-default.*")
UNLEASH_ENABLED = os.getenv("UNLEASH_ENABLED", "False").lower() == "true"
_DD_PROFILING_ENABLED = DD_TRACE_ENABLED and os.environ.get("DD_PROFILING_ENABLED", "false").lower() in ("true", "1", "yes")
...
def custom_openapi():
    ...
    openapi_schema = get_openapi(title="Custom title", version=APP_VERSION, summary="This is a very custom OpenAPI schema", description="Here's a longer description of the custom **OpenAPI** schema", routes=app.routes)
...
def _configure_unleash_feature_middleware(app: FastAPI) -> None:
    ...
    if UNLEASH_ENABLED or client.is_enabled("rate_limiter"):
        app.add_exception_handler(RateLimitExceeded, lambda r, e: JSONResponse(status_code=429, content={"error": "Too Many Requests"}))
    else:
        logger.warning("Feature flag : rate_limiter is not enabled")
    if UNLEASH_ENABLED or client.is_enabled("cors"):
        origins = ["http://localhost", "http://localhost:8080", "http://localhost:8091", "http://localhost:8001", "https://fastapi-sample.service.gra.dev.consul/", "https://fastapi-sample.service.gra.uat.consul/", "https://fastapi-sample.fastapicloud.dev/"]
        app.add_middleware(CORSMiddleware, allow_origins=origins, allow_credentials=True, allow_methods=["GET", "POST", "PUT"], allow_headers=["*"])
    else:
...
app = FastAPI(lifespan=combined_lifespan, title=APP_NAME + " " + APP_PREFIX_VERSION, description="FastAPI Sample for demo", version=APP_RUNTIME_VERSION, debug=os.getenv("DEBUG", "False").lower() == "true", default_response_class=ORJSONResponse)
initialize_api(app, logfire_enabled=configure_logfire(app, service_name=APP_NAME, service_version=APP_VERSION))
...
mcp = FastMCP.from_fastapi(app=app, name="mcp", route_map_fn=_mcp_openapi_route_filter)
...
async def metrics_middleware(request, call_next):
    ...
    REQUESTS_IN_PROGRESS.labels(method=request.method, path=request.url.path, app_name=APP_NAME).inc()
    REQUESTS.labels(method=request.method, path=request.url.path, app_name=APP_NAME).inc()
    try:
        response = await call_next(request)
        RESPONSES.labels(method=request.method, path=request.url.path, status_code=response.status_code, app_name=APP_NAME).inc()
        REQUESTS_PROCESSING_TIME.labels(method=request.method, path=request.url.path, app_name=APP_NAME).observe(time.time() - start_time)
        return response
    finally:
        INFLIGHT_REQUESTS.dec()
        REQUESTS_IN_PROGRESS.labels(method=request.method, path=request.url.path, app_name=APP_NAME).dec()
...
async def logging_middleware(request, call_next):
    ...
    try:
        ...
        logger.exception("request_failed", method=request.method, path=request.url.path, duration_seconds=time.time() - start_time)
        raise
    duration = time.time() - start_time
    log_fields = {"method": request.method, "path": request.url.path, "status_code": response.status_code, "duration_seconds": duration}
    noisy_success_paths = {"/docs", "/docs/oauth2-redirect", "/health", "/healthz", "/logs", "/metrics", "/openapi.json", "/ping", "/redoc", "/sickz", "/stream"}
...
user_id = "usr.id"
set_user(tracer, user_id, name="AlbanAndrieu", email=os.environ.get("MAIL_FROM", "alban.andrieu@gmail.com"), scope="sample_scope", role="manager", session_id="id_session", propagate=True)
configure_sentry()
...
if os.getenv("DEBUG"):
    import arel
    hot_reload = arel.HotReload(paths=[arel.Path("./nabla/data", on_reload=[reload_data]), arel.Path("./nabla/static"), arel.Path("./templates")])
...
@app.get("/api/data")
def get_sample_data():
    return {"data": [{"id": 1, "name": "Sample Item 1", "value": 100}, {"id": 2, "name": "Sample Item 2", "value": 200}, {"id": 3, "name": "Sample Item 3", "value": 300}], "total": 3, "timestamp": "2024-01-01T00:00:00Z"}


@app.get("/api/items/{item_id}")
def get_item(item_id: int):
    return {"item": {"id": item_id, "name": "Sample Item " + str(item_id), "value": item_id * 100}, "timestamp": "2024-01-01T00:00:00Z"}


@app.get("/api", response_class=HTMLResponse)
def read_root(request: Request):
    return render_api_root_page(title_suffix=os.getenv("TITLE_SUFFIX"), app_version=html.escape(str(request.app.version)))
...
@app.get("/sickz", response_class=ORJSONResponse, tags=["Health"], summary="Inverse reachability (sickz)")
...
async def trigger_error() -> JSONResponse:
    ...
    return JSONResponse(status_code=500, content={"error": "Intentional Sentry test error", "event_id": str(event_id) if event_id else None})
