...
from nabla.api.health_board_diagnostics import (
...
async def build_extended_healthz(
    ...
    *,
    ...
    return await _build_extended_healthz(
        request,
        reconciliation_context=reconciliation_context,
        deadline_seconds=_HEALTHZ_OPTIONAL_ENRICHMENT_DEADLINE_SEC,
    )
...
async def build_sickz_snapshot(request: Request) -> dict[str, Any]:
    return await _build_sickz_snapshot(
        request,
