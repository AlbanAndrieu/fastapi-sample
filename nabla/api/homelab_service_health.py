"""Pure per-service reconciliation for homelab health evidence."""
...
def _runtime_app_for_service(
    ...
    if binding is not None and binding.provider == "truenas-app":
        matches = [
            app for app in runtime.apps if match_runtime_binding(app, binding)[0]
        ]
...
def _reconciled_state(
    *,
    ...
    """Resolve service state using origin/runtime evidence before edge evidence."""
...
def _row_for_service(
    ...
    *,
    ...
    app: ObservedApp | None,
    tunnel_evidence: dict[str, str | None] | None,
    tunnel_health: HealthState | None,
    ...
    application_error = (
        ...
        if direct_result and direct_result.get("application_error")
        else None
    )
    ...
    state = _reconciled_state(
        ...
        direct_http_status=http_status,
        application_error=application_error is not None,
        runtime_missing=runtime_missing,
    )
    observed_at, age, stale = _observation_freshness(
    ...
    for prefix, result in (("direct", direct_result), ("internal", internal_result)):
        row[f"{prefix}_probe_source"] = result.get("probe_source") if result else None
        row[f"{prefix}_probe_observed_at"] = (
            result.get("probe_observed_at") if result else None
        )
        row[f"{prefix}_probe_age_seconds"] = (
            result.get("probe_age_seconds") if result else None
        )
        row[f"{prefix}_probe_refresh_error"] = (
            result.get("probe_refresh_error") if result else None
        )
...
def build_reconciled_service_health(
    ...
    *,
    ...
    """Reconcile service-local evidence without applying dependency propagation."""
    direct_by_url = {
        normalized: result
