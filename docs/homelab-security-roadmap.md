...
- **pfSense administration/API (`10443/tcp`)**: treat the public FastAPI Cloud probe as diagnostic-only while cloud egress is not a stable workload identity. Keep the listener `trusted_sources_only` for genuinely stable approved administration sources and move posture/Snort telemetry to an independent out-of-band observer; unrelated Internet origins remain denied.
