...
const expectedVersion = (process.env.EXPECTED_MASTER_VERSION || "").trim();

if (!/^\d+\.\d+\.\d+$/.test(expectedVersion)) {
  throw new Error(`EXPECTED_MASTER_VERSION must be a semantic version, got ${JSON.stringify(expectedVersion)}`);
}
...
try {
  ...
  console.log(`Master source version: ${expectedVersion}`);

  if (deployedVersion !== expectedVersion) {
    throw new Error(
      `Production deployment drift: UI reports ${deployedVersion}, but master declares ${expectedVersion}`,
    );
  }
} finally {
  await browser.close();
