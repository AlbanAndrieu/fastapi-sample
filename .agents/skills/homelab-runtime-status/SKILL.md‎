...
An agent must not declare a homelab migration, remediation, diagnostic, deployment, or validation complete while a known residual, deferred validation, limitation, cross-repository follow-up, or unresolved risk has not been recorded in `docs/engineering-roadmap.md`.

Before claiming completion, reconcile the observed evidence with the roadmap and ensure every remaining item includes the next acceptance proof. A partial success such as container `RUNNING`, TCP reachability, HTTP response, or dashboard green is not sufficient to discard an unresolved application-level or observer-level gap.
